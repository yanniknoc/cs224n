...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        output = self.pretrained_embeddings(t)
        x = output.view(t.shape[0], self.n_features * self.embed_size)
    ...
    def forward(self, t):
        ...
        fwd_embeddings = self.embedding_lookup(t)
        fwd_hidden = self.embed_to_hidden(fwd_embeddings)
        relu = nn.ReLU()
        fwd_relu = relu(fwd_hidden)
        fwd_dropout = self.dropout(fwd_relu)
        logits = self.hidden_to_logits(fwd_dropout)
        ### END YOUR CODE
        return logits
