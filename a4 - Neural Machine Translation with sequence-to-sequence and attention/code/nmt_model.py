...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=True) # (b, src_len, 2*h)
        
        # 3. Compute `dec_init_state`
        fwd_h, bwd_h = last_hidden[0], last_hidden[1] # (b, h)
        concatted_h = torch.cat((fwd_h, bwd_h), 1) # (b, 2*h)
        init_decoder_hidden = self.h_projection(concatted_h) # (b, 2*h)
        
        fwd_c, bwd_c = last_cell[0], last_cell[1] # (b, h)
        concatted_c = torch.cat((fwd_c, bwd_c), 1) # (b, 2*h)
        init_decoder_cell = self.c_projection(concatted_c) # (b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1 + 2. Apply decoder to `Ybar_t` and `dec_state`
        dec_hidden, dec_cell = self.decoder(Ybar_t, dec_state) # dec_hidden (b,h), dec_cell (b,h)
        dec_state = (dec_hidden, dec_cell)
        
        # 3. Compute attention scores
        dec_hidden = torch.unsqueeze(dec_hidden, dim=2) # (b,h,1)
        e_t = torch.bmm(input=enc_hiddens_proj, mat2=dec_hidden) # (b,src_len,1)
